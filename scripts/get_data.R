...
# clear environment -> load AZ files
rm(list = ls())
gc()
...
# clear environment -> load destinations
rm(list = ls())
gc()
...
trips$destid <- paste(
  trips$name, as.character(trips$lon), as.character(trips$lat), sep = '_')
destinations$destid <- paste(
  destinations$name, as.character(destinations$lon), as.character(destinations$lat), sep = '_')

# check that id values match between trips/destinations
length(unique(trips$destid))  
length(unique(destinations$destid)) # n = 4,523
